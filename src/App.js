  ...
  const updateSiteField = (region, id, field, value) => {
    ...
    if (field === "phase") {
      ...
      if (oldPhase !== value) {
        ...
        fbPush(`${region}/${id}/activityLog`, { action: `Phase: ${oldPhase} \u2192 ${value}`, ts: new Date().toISOString(), by: "User" });
      }
    }
  };
        ...
        {marketsOpen && (
          ...
          {tab === "dashboard" && (() => {
            ...
            {(() => {
              ...
              const phases = ["Prospect", "LOI Sent", "LOI Signed", "Under Contract"];
              const now = Date.now();
              const phaseStats = phases.map(ph => {
                const inPhase = allSites.filter(s => s.phase === ph);
                const avgDays = inPhase.length > 0 ? Math.round(inPhase.reduce((sum, s) => {
                  const hist = s.phaseHistory ? Object.values(s.phaseHistory) : [];
                  const entered = hist.find(h => h.to === ph);
                  const enteredAt = entered ? new Date(entered.changedAt).getTime() : (s.addedAt ? new Date(s.addedAt).getTime() : now - 30*86400000);
                  return sum + (now - enteredAt) / 86400000;
                }, 0) / inPhase.length) : 0;
                return { phase: ph, count: inPhase.length, avgDays };
              });
              const staleDays = 14;
              const stalled = allSites.filter(s => {
                if (["Closed", "Dead"].includes(s.phase)) return false;
                const hist = s.phaseHistory ? Object.values(s.phaseHistory) : [];
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }} onClick={() => setWeightEditorOpen(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%)', borderRadius: 16, border: '1px solid rgba(99,102,241,0.3)', padding: 32, width: 520, maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 25px 50px rgba(0,0,0,0.5), 0 0 40px rgba(99,102,241,0.1)' }}>
            ...
            <div style={{ background: 'rgba(99,102,241,0.1)', borderRadius: 10, padding: '10px 16px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid rgba(99,102,241,0.2)' }}>
              ...
              <span style={{ color: Math.abs(editWeights.reduce((s, w) => s + w.weight, 0) - 1) < 0.001 ? '#4ade80' : '#f87171', fontSize: 18, fontWeight: 700 }}>{Math.round(editWeights.reduce((s, w) => s + w.weight, 0) * 100)}%</span>
            </div>
