...
const SITE_IQ_DEFAULTS = [
  { key: "population", label: "Population", icon: "ð¥", weight: 0.18, tip: "3-mile population density", source: "ESRI / Census ACS", group: "demographics" },
  { key: "growth", label: "Growth", icon: "ð", weight: 0.13, tip: "Pop growth CAGR â 5yr projected trend", source: "ESRI 2025â2030 projections", group: "demographics" },
  { key: "income", label: "Med. Income", icon: "ð°", weight: 0.08, tip: "Median HHI within 3 miles", source: "ESRI / Census ACS", group: "demographics" },
  { key: "pricing", label: "Pricing", icon: "ð²", weight: 0.08, tip: "Price per acre vs. PS acquisition targets", source: "Asking price / acreage", group: "deal" },
  { key: "spacing", label: "PS Spacing", icon: "ð", weight: 0.20, tip: "Distance to nearest PS facility", source: "PS_Locations_ALL.csv", group: "proximity" },
  { key: "zoning", label: "Zoning", icon: "ð", weight: 0.13, tip: "By-right / conditional / prohibited", source: "Zoning field + summary", group: "entitlements" },
  ...
  { key: "competition", label: "Competition", icon: "ð¢", weight: 0.05, tip: "Storage competitor density", source: "Competitor data / summary", group: "market" },
  { key: "marketTier", label: "Market Tier", icon: "ð", weight: 0.08, tip: "PS market priority ranking", source: "Market field / config", group: "market" },
];
...
const generateVettingReport = (site, nearestPSDistance, iqResult) => {
  ...
  <div style="padding:24px 40px 40px">
    ...
    <table>${[
      ...
      row("PS Internal Price", site.internalPrice || "â"),
    ...
    <table>${[
      ...
      row("Storage Use", site.zoning ? "Verify with local jurisdiction" : "UNKNOWN â research required"),
    ].join("")}</table>
...
const computeSiteIQ = (site) => {
  ...
  // --- 2c. PRICING (8%) â Price per acre vs. PS acquisition targets ---
  ...
  return {
    ...
    label: final >= 9 ? "ELITE" : fddress || ""}, ${site.city || ""}, ${site.state || ""} &nbsp;|&nbsp; ${new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</div>
      </div>
      <div style="text-align:right">
        <div style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:10px;background:${iqBadgeColor}18;border:1px solid ${iqBadgeColor}40">
          <span style="font-size:28px;font-weight:900;color:${iqBadgeColor};font-family:'Space Mono'">${typeof iqScore === "number" ? iqScore.toFixed(1) : iqScore}</span>
...
export default function App() {
  ...
  // --- AUTH GATE + SESSION LOGGING ---
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (user) => {
      setAuthUser(user);
      setAuthLoading(false);
      if (user) logSession(user);
    });
    return () => unsub();
  }, []);
  ...
  // --- PAGE VIEW LOGGING ---
  useEffect(() => {
    if (authUser) logPageView(authUser, tab);
  }, [tab, authUser]);
  ...
  // Computes SiteIQ once pddress || ""}, ${site.city || ""}, ${site.state || ""} &nbsp;|&nbsp; ${new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</div>
      </div>
      <div style="text-align:right">
        <div style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:10px;background:${iqBadgeColor}18;border:1px solid ${iqBadgeColor}40">
          <span style="font-size:28px;font-weight:900;color:${iqBadgeColor};font-family:'Space Mono'">${typeof iqScore === "number" ? iqScore.toFixed(1) : iqScore}</span>
...
export default function App() {
                    ...
                    <div className="card-expand" style={{ padding: "0 18px 18px", borderTop: "2px solid transparent", borderImage: "linear-gradient(90deg, transparent, #D45500, #F37C33, #FFB347, #F37C33, #D45500, transparent) 1" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 280 }}>
                            ...
                            {showSiteIQDetail[site.id] && (() => {
                              ...
                              const dims = [
                                ...
                                { key: "pricing", label: "Price / Acre", weight: getIQWeight("pricing"), icon: "ð·ï¸", tip: "Asking price per acre vs PS targets" },
                                { key: "spacing", label: "PS Spacing", weight: getIQWeight("spacing"), icon: "ð", tip: "Distance to nearest Public Storage" },
                      ...
                      {(site.pop3mi || site.income3mi) && (() => {
                        ...
                        const rows = [
                          ...
                          { label: "Nearest PS", val: site.siteiqData?.nearestPS ? site.siteiqData.nearestPS.toFixed(1) + " mi" : null, icon: "ð" },
  ...
  // --- AUTH GATE ---
  if (authLoading) return <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", background: "#0B1120", color: "#94A3B8", fontSize: 16, fontWeight: 600 }}>Loading...</div>;
  if (!authUser) return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", background: "linear-gradient(135deg, #0B1120 0%, #1E293B 100%)" }}>
      <form onSubmit={handleLogin} style={{ background: "#0F172A", borderRadius: 16, padding: "40px 36px", border: "1px solid #1E293B", width: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: "#E2E8F0", letterSpacing: -0.5 }}>PS Tracker</div>
          <div style={{ fontSize: 12, color: "#64748B", marginTop: 6 }}>Sign in to continue</div>
        </div>
        {loginError && <div style={{ background: "#7F1D1D", color: "#FCA5A5", padding: "8px 12px", borderRadius: 8, fontSize: 12, marginBottom: 14, textAlign: "center" }}>{loginError}</div>}
        <input type="email" placeholder="Email" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} required style={{ width: "100%", padding: "12px 14px", borderRadius: 8, border: "1px solid #334155", background: "#1E293B", color: "#E2E8F0", fontSize: 14, marginBottom: 12, outline: "none", boxSizing: "border-box" }} />
        <input type="password" placeholder="Password" value={loginPass} onChange={e => setLoginPass(e.target.value)} required style={{ width: "100%", padding: "12px 14px", borderRadius: 8, border: "1px solid #334155", background: "#1E293B", color: "#E2E8F0", fontSize: 14, marginBottom: 20, outline: "none", boxSizing: "border-box" }} />
        <button type="submit" disabled={loginBusy} style={{ width: "100%", padding: "12px", borderRadius: 8, border: "none", background: loginBusy ? "#334155" : "linear-gradient(135deg, #2563EB 0%, #3B82F6 100%)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: loginBusy ? "not-allowed" : "pointer" }}>{loginBusy ? "Signing in..." : "Sign In"}</button>
      </form>
    </div>
  );
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: "#94A3B8", letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 1 }}>Acquisition Pipeline <span style={{ color: "#F37C33", fontWeight: 700 }}>Â·</span> <span style={{ fontWeight: 800, color: "#F37C33" }}>4.0</span></div>
        ...
        { key: "activity", label: "Activity", adminOnly: true },
            ...
            >
              ...
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 11, color: "#64748B" }}>{authUser?.email?.split("@")[0]}</span>
                <button onClick={() => signOut(auth)} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #334155", background: "transparent", color: "#94A3B8", fontSize: 11, cursor: "pointer", fontWeight: 600 }}>Sign Out</button>
              </div>
        </div>
      </div>
        ...
        {tab === "dashboard" && (
          <div style={{ animation: "fadeIn 0.3s ease-out" }}>
            ...
            {(() => {
              ...
              const moveIcon = (to) => advancePhases.includes(to) ? "ð¢" : to === "Dead" || to === "PS Declined" ? "ð´" : "ðµ";
            ...
            {(() => {
              ...
              const funnelStages = [
                ...
                { label: "Submitted to PS", count: all.filter(s => s.phase === "Submitted to PS" || s.phase === "PS Revisions").length, color: "#6366F1", icon: "ð¤", action: () => navigateTo("summary", { phase: "Submitted to PS" }) },
                { label: "PS Approved", count: all.filter(s => s.phase === "PS Approved").length, color: "#8B5CF6", icon: "â", action: () => navigateTo("summary", { phase: "PS Approved" }) },
            ...
            {subs.length === 0 && [...sw, ...east].filter(s => s.assignedTo && s.needsReview).length === 0 ? (
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {sortData(subs).map((site) => {
                  ...
                  return (
    authLoading ? (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", background: "#0B1120", color: "#94A3B8", fontSize: 16, fontWeight: 600 }}>Loading...</div>
    ) : !authUser ? (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", background: "linear-gradient(135deg, #0B1120 0%, #1E293B 100%)" }}>
        <form onSubmit={handleLogin} style={{ background: "#0F172A", borderRadius: 16, padding: "40px 36px", border: "1px solid #1E293B", width: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
