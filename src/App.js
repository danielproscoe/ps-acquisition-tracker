...
import { computeSiteScore as _computeSiteScore, computeSiteFinancials, computeOptimalLayout, computeVettingIntel, buildSitePackage, computeValidationStats, parsePrice } from './scoring';
import {
...
function AppInner() {
  ...
  useEffect(() => {
    const parseCsv = (csv, ownership) => {
      ...
      const locs = [];
      // 3rdParty/Combined/NSA CSVs have 8 cols (includes ZIP at index 5), owned has 7
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 6, marginTop: 4, marginBottom: 4, flexWrap: "wrap" }}>
                        {site.listingUrl ? <a href={site.listingUrl.startsWith("http") ? site.listingUrl : `https://${site.listingUrl}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} style={{ padding: "2px 8px", borderRadius: 5, background: "rgba(201,168,76,0.06)", color: "#C9A84C", fontSize: 10, fontWeight: 600, textDecoration: "none", border: "1px solid rgba(201,168,76,0.12)" }}>Listing</a> : <span onClick={(e) => e.stopPropagation()} style={{ padding: "2px 8px", borderRadius: 5, background: "rgba(34,197,94,0.08)", color: "#22C55E", fontSize: 10, fontWeight: 700, border: "1px solid rgba(34,197,94,0.18)" }}>Off-Market</span>}
