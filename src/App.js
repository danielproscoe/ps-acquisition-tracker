          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div className="logo-spin-container" onClick={(e) => { e.currentTarget.querySelector('.logo-img')?.classList.remove('logo-click-spin'); void e.currentTarget.querySelector('.logo-img')?.offsetWidth; e.currentTarget.querySelector('.logo-img')?.classList.add('logo-click-spin'); }} style={{ width: 48, height: 48, borderRadius: 12, cursor: "pointer", position: "relative", boxShadow: "0 4px 20px rgba(232,122,46,0.25), 0 0 0 1px rgba(201,168,76,0.15)", flexShrink: 0 }}>
                ...
                {/* ═══ POWERHOUSE EFFECT LAYERS (8 layers) ═══ */}
                {/* Idle: Subtle breathing glow — always present */}
                <div className="logo-idle-glow" />
                {/* L8: Danger Flash — crimson alarm strobe */}
                <div className="logo-danger-flash" />
                {/* L7: Glitch — digital corruption flicker */}
                <div className="logo-glitch" />
                {/* L6: Reactor Containment Ring — industrial menace */}
                <div className="logo-reactor" />
                {/* L5: EMP Shockwave — expanding rings on peak moments */}
                <div className="logo-shockwave" />
                {/* L4.5: Heartbeat — SAW-style double-beat pulse */}
                <div className="logo-heartbeat" />
                {/* L3: Plasma Sheath — radial energy field with conic rotation */}
                <div className="logo-plasma" />
                {/* L2.5: Razor Wire Wrap — animated wire tightening */}
                <div className="logo-razorwire">
                  <svg viewBox="0 0 68 68" xmlns="http://www.w3.org/2000/svg">
                    {/* Spiral razor wire path wrapping the icon */}
                    <path className="razor-path" d="M34,2 C50,2 66,18 66,34 C66,50 50,66 34,66 C18,66 2,50 2,34 C2,18 18,2 34,2" fill="none" stroke="rgba(180,180,180,0.6)" strokeWidth="0.8" strokeLinecap="round" />
                    {/* Barb points along the wire */}
                    {[0, 45, 90, 135, 180, 225, 270, 315].map((deg, i) => {
                      const rad = deg * Math.PI / 180;
                      const r = 32;
                      const cx = 34 + r * Math.cos(rad);
                      const cy = 34 + r * Math.sin(rad);
                      const barbR = 4;
                      const a1 = rad - 0.8;
                      const a2 = rad + 0.8;
                      return (
                        <g key={i} className="spike-barb">
                          <line x1={cx} y1={cy} x2={cx + barbR * Math.cos(a1)} y2={cy + barbR * Math.sin(a1)} stroke="rgba(200,200,200,0.7)" strokeWidth="0.8" strokeLinecap="round" />
                          <line x1={cx} y1={cy} x2={cx + barbR * Math.cos(a2)} y2={cy + barbR * Math.sin(a2)} stroke="rgba(200,200,200,0.7)" strokeWidth="0.8" strokeLinecap="round" />
                        </g>
                      );
                    })}
                    {/* Second spiral — counter-rotating */}
                    <path className="razor-path" d="M34,6 C48,6 62,20 62,34 C62,48 48,62 34,62 C20,62 6,48 6,34 C6,20 20,6 34,6" fill="none" stroke="rgba(220,38,38,0.35)" strokeWidth="0.5" strokeLinecap="round" style={{ animationDelay: '0.3s' }} />
                  </svg>
                </div>
                {/* L2: Electric Arc Ring — dashed lightning rings */}
                <div className="logo-arc-ring" />
                {/* L1: Barbed Wire Spike Corona — razor spikes + barbs */}
                <div className="logo-spike-ring">
                  <svg viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
                    {/* 32 razor spikes with barbed wire cross-spikes */}
                    {Array.from({ length: 32 }, (_, i) => {
                      const angle = (i * 11.25) * (Math.PI / 180);
                      const inner = 22;
                      // Varying spike lengths — every 4th is a mega-spike
                      const outer = i % 4 === 0 ? 40 : i % 3 === 0 ? 36 : i % 2 === 0 ? 33 : 30;
                      const barbLen = i % 4 === 0 ? 6 : i % 3 === 0 ? 4 : 3;
                      const cx = 40, cy = 40;
                      const x1 = cx + inner * Math.cos(angle);
                      const y1 = cy + inner * Math.sin(angle);
                      const x2 = cx + outer * Math.cos(angle);
                      const y2 = cy + outer * Math.sin(angle);
                      // Barb cross-spike at 2/3 point
                      const barbAngle = angle + Math.PI / 2;
                      const midR = inner + (outer - inner) * 0.65;
                      const bx = cx + midR * Math.cos(angle);
                      const by = cy + midR * Math.sin(angle);
                      const bx1 = bx + barbLen * Math.cos(barbAngle);
                      const by1 = by + barbLen * Math.sin(barbAngle);
                      const bx2 = bx - barbLen * Math.cos(barbAngle);
                      const by2 = by - barbLen * Math.sin(barbAngle);
                      // Second barb at 1/3 point for mega-spikes
                      const midR2 = inner + (outer - inner) * 0.35;
                      const bx3 = cx + midR2 * Math.cos(angle);
                      const by3 = cy + midR2 * Math.sin(angle);
                      const barbLen2 = barbLen * 0.6;
                      const bx4 = bx3 + barbLen2 * Math.cos(barbAngle);
                      const by4 = by3 + barbLen2 * Math.sin(barbAngle);
                      const bx5 = bx3 - barbLen2 * Math.cos(barbAngle);
                      const by5 = by3 - barbLen2 * Math.sin(barbAngle);
                      const colors = ["#39FF14", "#00E5FF", "#E87A2E", "#FFD700", "#39FF14", "#00E5FF", "#E87A2E", "#C9A84C"];
                      const color = colors[i % 8];
                      const isMega = i % 4 === 0;
                      return (
                        <g key={i} className="spike-barb">
                          {/* Main spike */}
                          <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={color} strokeWidth={isMega ? 2 : i % 2 === 0 ? 1.2 : 0.8} strokeLinecap="round" opacity={isMega ? 1 : 0.8} />
                          {/* Primary barb */}
                          <line x1={bx1} y1={by1} x2={bx2} y2={by2} stroke={color} strokeWidth={isMega ? 1.2 : 0.7} strokeLinecap="round" opacity={0.7} />
                          {/* Secondary barb (mega-spikes only) */}
                          {isMega && <line x1={bx4} y1={by4} x2={bx5} y2={by5} stroke={color} strokeWidth={0.6} strokeLinecap="round" opacity={0.5} />}
                          {/* Spike tip glow dot (mega-spikes) */}
                          {isMega && <circle cx={x2} cy={y2} r={1.5} fill={color} opacity={0.9} />}
                        </g>
                      );
                    })}
                    {/* Inner twisted wire ring */}
                    <circle cx="40" cy="40" r="22" fill="none" stroke="rgba(57,255,20,0.5)" strokeWidth="0.7" strokeDasharray="3 2 1 2" />
                    {/* Mid barbed ring */}
                    <circle cx="40" cy="40" r="30" fill="none" stroke="rgba(0,229,255,0.3)" strokeWidth="0.4" strokeDasharray="1 3 2 3" />
                    {/* Outer perimeter ring */}
                    <circle cx="40" cy="40" r="38" fill="none" stroke="rgba(232,122,46,0.2)" strokeWidth="0.3" strokeDasharray="1 5" />
                    {/* Electric arc paths between select spikes */}
                    <path d="M40,2 Q50,12 58,8" fill="none" stroke="rgba(57,255,20,0.4)" strokeWidth="0.5" className="spike-barb" />
                    <path d="M72,28 Q68,38 76,40" fill="none" stroke="rgba(0,229,255,0.3)" strokeWidth="0.4" className="spike-barb" />
                    <path d="M58,72 Q48,68 52,78" fill="none" stroke="rgba(232,122,46,0.35)" strokeWidth="0.4" className="spike-barb" />
                    <path d="M8,52 Q12,42 4,40" fill="none" stroke="rgba(57,255,20,0.3)" strokeWidth="0.4" className="spike-barb" />
                  </svg>
                </div>
                {/* L4: Spark Particles — ejected during peak phases */}
                <div className="logo-particles">
                  <div className="spark" /><div className="spark" /><div className="spark" /><div className="spark" />
                  <div className="spark" /><div className="spark" /><div className="spark" /><div className="spark" />
                </div>
              </div>
              <div>
